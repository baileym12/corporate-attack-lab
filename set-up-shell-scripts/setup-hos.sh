...
# MedSecure Hospital Lab Build Script
# Must be run as root
...
DEPARTMENTS=("Medical" "IT" "HR" "Finance" "Administration")
...
USERS_PER_DEPT=( ["Medical"]=30 ["IT"]=10 ["HR"]=5 ["Finance"]=8 ["Administration"]=7 )
...
for dept in "${DEPARTMENTS[@]}"; do
    ...
    for i in $(seq 1 $count); do
        ...
        echo "$USERNAME ($FIRST $LAST) - $dept - $PASSWORD" >> /root/medsecure_user_credentials.txt
    done
done
...
for dept in "${DEPARTMENTS[@]}"; do
    for i in {1..5}; do
        echo "Patient record ${i} for $dept department." > "$BASE_DIR/$dept/${dept}_Record_${i}.txt"
        chmod 660 "$BASE_DIR/$dept/${dept}_Record_${i}.txt"
    done
done

echo "Configuring Nginx internal portal..."
echo "<html><body><h1>MedSecure Hospital Internal Portal</h1></body></html>" > /var/www/html/index.html
systemctl restart nginx

echo "COMPLETE: MedSecure Hospital server fully provisioned."
echo "User credentials saved to: /root/medsecure_user_credentials.txt"
