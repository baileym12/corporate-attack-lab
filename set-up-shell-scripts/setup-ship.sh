...
# GlobalShip Inc. Lab Build Script
# Must be run as root
...
DEPARTMENTS=("Operations" "Logistics" "IT" "HR" "Finance")
...
for dept in "${DEPARTMENTS[@]}"; do
    for i in $(seq 1 $USERS_PER_DEPT); do
        ...
        echo "$USERNAME ($FIRST $LAST) - $dept - $PASSWORD" >> /root/globalship_user_credentials.txt
    done
done
...
for dept in "${DEPARTMENTS[@]}"; do
    for i in {1..5}; do
        echo "Shipping report ${i} for $dept department." > "$BASE_DIR/$dept/${dept}_Report_${i}.txt"
        chmod 660 "$BASE_DIR/$dept/${dept}_Report_${i}.txt"
    done
done

echo "Configuring Nginx internal portal..."
echo "<html><body><h1>GlobalShip Internal Portal</h1></body></html>" > /var/www/html/index.html
systemctl restart nginx

echo "COMPLETE: GlobalShip server fully provisioned."
echo "User credentials saved to: /root/globalship_user_credentials.txt"
